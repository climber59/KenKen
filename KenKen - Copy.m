...
hitting restart after finishing a puzzle does not remove the check mark

hitting fill singles after winning adds more check marks
...
'X' button in note mode clears all notes?
-currently just closes the tool

Change number of columns in enter tool in larger grids?
-fine at 9, ridiculous at 20

uitabgroup(),uitab() for advanced options
===================================== Rule changes
...
function [] = KenKen()
	...
	gridSize = [];
	finished = [];
	but = []; %matlab.ui.control.UIControl;
	boxes = [];
	allowedOps = [];
	numPicker = [];
	addCheck = [];
...
% 		num = str2num(newNum);
		...
		if noteBtn.Value
			...
			textGrid(r,c).String = '';
			if num > 0 % change a specific number
				if strcmp(notesGrid(r,c,num).Visible,'on')
					notesGrid(r,c,num).Visible = 'off';
				else
					notesGrid(r,c,num).Visible = 'on';
				end
			elseif num == -1 % enter all possible notes (based on latin square)
				for i = 1:n
...
% 		ax.Position = [0.5*(1-f.Position(4)/f.Position(3)) 0 1*f.Position(4)/f.Position(3) 1];
		...
		factorInput = uicontrol(...
			...
			'Style','edit',...
			'String','12',...
			'Position',[0.025 0.85 0.05 0.075],...
			'FontSize',15);	
		factorDisp = uicontrol(...
			...
			'Style','text',...
			'String','2 2 3',...
			'Position',[0.025 0.725 0.105 0.1],...
			'FontSize',15);
		factorBtn = uicontrol(...
			'Parent',f,...
			'Units','normalized',...
			'Style','pushbutton',...
			'String','Factor',...
			'Callback',{@primeFac, factorInput, factorDisp},...
			'Position',[0.08 0.85 0.05 0.075],...
			'FontSize',15,...
...
% 		[0.01+sum(blobLbl.Position([1,3])) blobLbl.Position(2)+0.01 0.25 0.05]
% 		[0.0 0.9 0.5 0.05]
		gridSize = uicontrol(...
			'Parent',genOptions,...
			'Units','normalized',...
		...
		gridLbl = uicontrol(...
			...
			'FontSize',9,...
			'FontUnits','normalized');
% 		[0.01+sum(gridLbl.Position([1,3])) gridLbl.Position(2)+0.01 0.25 0.05]
% 		[0.0 0.8 0.5 0.05]
		
		allowedOps = true(1,4);
		opPickerPanel = uipanel(... % will need special resizing code
			'Parent',genOptions,...
			'Units','normalized',...
...
% 			'Parent',genOptions,...
% 			'Units','normalized',...
% 			'Position',[0.02 0.7 0.96 0.105],...
% 			'Data',logical([1 1 1 1]),...
% 			'ColumnEditable',true,...
% 			'ColumnWidth',{20 20 20 20},...
% 			'ColumnName',{'+','-','*','/'},...
% 			'RowName',{});
% 		opPicker.Position([3 4]) = opPicker.Extent([3 4]);
% 		opPicker.ColumnWidth = 'auto';
	...
	function [] = opSelection(src, ~, op)
		...
		elseif allowedOps(4) && all(~allowedOps(1:3))
			% only allow if the array is all powers of x and blob size is
			% limited to 2
			x = sort(numPicker.Data); % sort will probably end up being redundant
			x = x(2:end)./x(1:end-1);
			if all(x(1) == x)
				blobSize.String = '2';
				blobClip();
...
% 		evt
		...
		if isnan(evt.NewData)
			if ~isnan(str2num(evt.EditData)) %#ok<*ST2NM>
...
% 		arrayOptions.SelectedObject.Callback
	...
	function [] = showNums()
		for r = 1:n
			for c = 1:n
				text(c+0.5,r+0.5,num2str(numGrid(r,c)),'FontSize',20,'HorizontalAlignment','center');
			end
		end
	end
	...
	function [] = opShower()
		...
		for i = 1:length(blobs)
			...
			blobs(i).UserData.opT = text(0.1+c,0.15+r,sprintf('%s %d',ops(blobs(i).UserData.op), blobs(i).UserData.ans));
			drawnow;
		end
	end
	...
	function [] = opChooser()
		for i = 1:length(blobs)
			...
			elseif s==2
				...
				if mod(num(1),num(2))==0 || mod(num(2),num(1))==0
					blobs(i).UserData.op = randi([2 5]);
				else
					blobs(i).UserData.op = randi([2 4]);
				end
			else
				blobs(i).UserData.op = randi(2)*2;
			end
	...
	function [grid, good] = recGen(grid, r, c)
		...
		e = 1:n;
		e(w) = []; % only currently valid options
