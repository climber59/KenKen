...
several ui scaling factors are hardcoded. I would prefer them all defined
in one place
-ex, big numbers are allowed 0.7 of a box
...
-also possible center the height rather than appearing at the corner
-way harder than I thought. panel position is relative to the figure, but
the boxes are relative to the axes and the axes extends beyond what is
shown depending on the aspect ratio
...
-currently useful with the overlapping notes.
...
% 							but(butInd).String
% 							but(butInd).UserData.cellRow
% 							but(butInd).UserData.strInds
% 							u = ;
...
% 					end
	...
	function [] = resize(~,~)
% % 		f.Visible = 'off';
% 		numPicker.Units = 'normalized';
% 		numPicker.Position = [0.02 0.02 (-0.04 + arrayOptions.Position(1)) arrayOptions.Position(4)];
% 		numPicker.Units = 'pixels';
% % 		ap = arrayOptions.Position
% % 		p = numPicker.Position
% % 		e = numPicker.Extent
% 		x = 3 + 16*(numPicker.Extent(4) > numPicker.Position(4));
% 		numPicker.ColumnWidth = {numPicker.Position(3)-x};
% % 		f.Visible = 'on';
% 		drawnow;
	end
	...
	function [] = click(~,~)
		...
		axesPos = ax.Position%([2 4])
		axesOuterPos = ax.OuterPosition%([2 4])
		m = m
		slope = ax.Position(4) + ax.OuterPosition(2)%(ax.OuterPosition(4)-ax.Position(2))/n
		x = m(2)*(ax.OuterPosition(3)-ax.Position(1))/n + ax.Position(1)
		y = (n-m(1))*(ax.Position(4) + ax.OuterPosition(2))/n + ax.Position(2)
		
		ax.Units = 'normalized';
		% May need code for the two cases: limited by height or width.
		% maybe try setting f.Units to 'normalized'. may play nicer with
		% the axes
		%}
		numPanel.Position(1) = f.CurrentPoint(1);
		numPanel.Position(2) = max(0, min([f.CurrentPoint(2)-numPanel.Position(4)/2, f.Position(4) - numPanel.Position(4)]));
	...
	function [] = numFill(~,~,newNumStr, num, cellRow, strInds)
		...
		if noteMode
			...
			if ~isnan(num)% change a specific number
				noteChange(notesGrid(r,c).String{cellRow}(strInds(1)) == ' ');
...
% 		showNums(); % For debugging purposes only
	...
	function [] = buildEnterTool()
		...
		a = 1-1/nR;
		for i = 1:n
			[ri, ci] = regexp(notesGrid(1,1).UserData.all,num2str(theNums(i)));
			j = 1;
			while isempty(ri{j}) && j < length(ri) %in theory the j< check is unnecessary
				j = j + 1;
			end
			...
			if i > length(but) - 2
				...
				'FontSize',15,...
				'Callback',{@numFill, num2str(theNums(i)), theNums(i), j, ri{j}:ci{j}});
			else
				but(i+2).String = num2str(theNums(i));
				but(i+2).Callback = {@numFill, num2str(theNums(i)), theNums(i), j, ri{j}:ci{j}};
			end
			but(i+2).UserData.cellRow = j;
			but(i+2).UserData.strInds = ri{j}:ci{j};
...
% 		arrayOptions.SelectedObject.Callback
	...
	function [] = blankNums()
		...
		while ~shortEnough
			...
			shortEnough = (notesGrid(1,1).Extent(4) < 0.8);
			if ~shortEnough
				fs = fs * 0.9;
			end
		end
		none = regexprep(noteString,'\d',' ');
		
% 		longest = ceil(log10(max(theNums)));
		
% 		temp = text(1.5,1.575,num2str(max(theNums)),'FontUnits','normalized','FontSize',0.7/n,'HorizontalAlignment','center','VerticalAlignment','middle');
% 		if temp.Extent(3) > 1
% 			
% 		end
		for r = 1:n
			for c = 1:n
				textGrid(r,c) = text(c+0.5,r+0.575,' ','FontUnits','normalized','FontSize',0.7/n,'HorizontalAlignment','center','VerticalAlignment','middle');
...
% 				end
	...
	function [] = opShower()
		...
		fs = 0.1;
		temp = text(0,0,sprintf('%s %d',ops(blobs(i).UserData.op), blobs(i).UserData.ans),'FontUnits','normalized','FontSize',fs,'FontName','fixedwidth','FontWeight','bold');
% 		temp.Extent
		fs = min(0.85*fs/temp.Extent(3),0.3*fs/temp.Extent(4));
		temp.FontSize = fs;
% 		temp.Extent
		delete(temp);
		for i = 1:length(blobs)
			...
			blobs(i).UserData.opT = text(0.1+c,0.15+r,sprintf('%s %d',ops(blobs(i).UserData.op), blobs(i).UserData.ans),'FontUnits','normalized','FontSize',fs,'FontName','fixedwidth','FontWeight','bold');
			drawnow;
		end
	end
...
% 			blobs(i).UserData.op
	...
	function [] = patchGrid(n)
		gridlines = gobjects(2*n,1);%matlab.graphics.primitive.Line;
		for i = 1:n
			gridlines(i) = line([i i], [1 n+1],'Color',0.5*ones(1,3),'Visible','on');
			gridlines(i+n) = line([1 n+1], [i i],'Color',0.5*ones(1,3),'Visible','on');
	...
	function [] = figureSetup()
		...
		arrayOptions = uibuttongroup(...
			'Parent',genOptions,...
			'Units','normalized',...
			'Position',[0.02 0.02 0.55 0.5]);
% 		arrayOptions.Position(3) = 1 - arrayOptions.Position(1);
		
		numPicker = uitable(...
			'Parent',genOptions,...
			'Units','normalized',...
			'Position',[0.57 0.02 0.48 0.5],...
...
% 			'ColumnWidth',{30},...
		...
		numPicker.Position(3) = min(x-0.01,0.5);
