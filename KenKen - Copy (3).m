...
% 					end
	...
	function [] = numFill(~,~,newNumStr, num, numInd, cellRow, strInds)
...
% 				strInds
% 				notesGrid(r,c).String{cellRow}(strInds)
	...
	function [] = buildEnterTool()
		...
		for i = 1:n
			...
			if i > length(but) - 2
				...
				'FontSize',15,...
				'Callback',{@numFill, num2str(theNums(i)), theNums(i), i, j, ri{j}:ci{j}});
			else
				but(i+2).String = num2str(theNums(i));
				but(i+2).Callback = {@numFill, num2str(theNums(i)), theNums(i), i, j, ri{j}:ci{j}};
			end
